...
public class AstVisitor<T> {
    ...
    public T visitPartitionByClause(PartitionByClause partitionByClause) {
        if (null != partitionByClause.getExprs()) {
            for (ColumnExpr expr : partitionByClause.getExprs()) {
    ...
    public T visitSelectColumnExprList(List<ColumnExpr> exprs, boolean isDistinct) {
    ...
    public T visitLambdaColumnExpr(LambdaColumnExpr expr) {
        return null;
    }
    ...
    public T visitWinFunctionColumnExpr(WinFunctionColumnExpr expr) {
        if (null != expr.getWindowFunction()) {
            visitIdentifier(expr.getWindowFunction());
        }
        if (null != expr.getWindowExpr()) {
            visitWindowExpr(expr.getWindowExpr());
        }
        return null;
    }

    public T visitWindowExpr(WindowExpr expr){
        if (null != expr.getPartitionByClause()) {
            visitPartitionByClause(expr.getPartitionByClause());
        }
        if (null != expr.getPartitionByClause()) {
            visitOrderByClause(expr.getOrderByClause());
        }
        return null;
    }
    public T visitWinFunctionTargetColumnExpr(WinFunctionTargetColumnExpr expr) {
        if (null != expr.getWindowFunction()) {
            visitIdentifier(expr.getWindowFunction());
        }
        if (null != expr.getColumnExprs()) {
            visitColumnExprList(expr.getColumnExprs());
        }
        if (null != expr.getWindowName()) {
            visitIdentifier(expr.getWindowName());
        }
        return null;
    }
    ...
    public T visitDescribeQuery(DescribeQuery describeQuery) {
        if( null != describeQuery.getTableExpr()){
            visit(describeQuery.getTableExpr());
        }
        return null;
    }

    public T visitShowTableQuery(ShowTableQuery showTableQuery) {
        return null;
    }

    public T visitShowDatabaseQuery(ShowDatabaseQuery showDatabaseQuery) {
        return null;
    }

    public T visitWindowClause(WindowClause windowClause) {
        if (null != windowClause.getWindowName()) {
            visitIdentifier(windowClause.getWindowName());
        }

        if (null != windowClause.getWindowExpr()) {
            visitWindowExpr(windowClause.getWindowExpr());
        }
        return null;
    }
}
