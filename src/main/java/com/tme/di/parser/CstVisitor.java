...
public class CstVisitor extends ClickHouseParserBaseVisitor {
    ...
    public Object visitAlterTableClauseAddColumn(ClickHouseParser.AlterTableClauseAddColumnContext ctx) {
        ...
        boolean ifNotExists = null != ctx.IF();
...
//    }
    ...
    public Object visitDescribeStmt(ClickHouseParser.DescribeStmtContext ctx) {
        ClickHouseParser.TableExprContext tableExprContext = ctx.tableExpr();
        TableExpr tableExpr = (TableExpr) visit(tableExprContext);
        DescribeQuery describeQuery = new DescribeQuery(tableExpr);
        return describeQuery;
    }

    @Override
    public Object visitShowTablesStmt(ClickHouseParser.ShowTablesStmtContext ctx) {
        ShowTableQuery showTableQuery = new ShowTableQuery();
        return showTableQuery;
    }

    @Override
    public Object visitShowDatabasesStmt(ClickHouseParser.ShowDatabasesStmtContext ctx) {
        ShowDatabaseQuery showDatabaseQuery = new ShowDatabaseQuery();
    ...
    public Object visitSelectStmt(ClickHouseParser.SelectStmtContext ctx) {
        ...
        SelectStatement selectStatement = new SelectStatement(distinct, type, withTotals);
        ...
        if (null != ctx.windowClause()) {
            WindowClause windowClause = (WindowClause) visit(ctx.windowClause());
            selectStatement.setWindowClause(windowClause);
        }
    ...
    public Object visitWindowExpr(ClickHouseParser.WindowExprContext ctx) {
        ...
        if(ctx.winPartitionByClause() != null){
            partitionByClause = (PartitionByClause) visit(ctx.winPartitionByClause());
        }
        OrderByClause orderByClause = null;
        if(ctx.winOrderByClause() != null){
            orderByClause = (OrderByClause) visit(ctx.winOrderByClause());
        }
        return new WindowExpr(partitionByClause, orderByClause);
    }

    @Override
    public Object visitWinPartitionByClause(ClickHouseParser.WinPartitionByClauseContext ctx) {
        List<ColumnExpr> columnExprs = (List<ColumnExpr>) visit(ctx.columnExprList());
        PartitionByClause partitionByClause = new PartitionByClause(columnExprs);
        return partitionByClause;
    }

    @Override
    public Object visitWinOrderByClause(ClickHouseParser.WinOrderByClauseContext ctx) {
        List<OrderExpr> orderByExprs = (List<OrderExpr>) ctx.orderExprList();
        OrderByClause orderByClause = new OrderByClause(orderByExprs);
        return orderByClause;
    }

    @Override
    public Object visitWindowClause(ClickHouseParser.WindowClauseContext ctx) {
        Identifier windowName = (Identifier) visitIdentifier(ctx.identifier());
        WindowExpr windowExpr = (WindowExpr) visitWindowExpr(ctx.windowExpr());
        WindowClause windowClause = new WindowClause(windowName, windowExpr);
        return windowClause;
    }
    ...
    public Object visitColumnExprWinFunctionTarget(ClickHouseParser.ColumnExprWinFunctionTargetContext ctx) {
        if(ctx.identifier().size() != 2){
            ...
            Identifier functionName = (Identifier) visit(ctx.identifier(0));
            Identifier windowName = (Identifier) visit(ctx.identifier(1));
            List<ColumnExpr> columnExprs = (List<ColumnExpr>) visit(ctx.columnExprList());
            return ColumnExpr.createWinFunctionTarget(functionName, columnExprs, windowName);
        }
    }
    ...
    public Object visitColumnExprWinFunction(ClickHouseParser.ColumnExprWinFunctionContext ctx) {
        Identifier identifier = (Identifier) visit(ctx.identifier());
        ClickHouseParser.WindowExprContext winCtx = ctx.windowExpr();
        PartitionByClause partitionByClause = null;
        OrderByClause orderByClause = null;
        if(null != winCtx){
            ClickHouseParser.WinPartitionByClauseContext partitionByClauseContext = ctx.windowExpr().winPartitionByClause();
            if(null != partitionByClauseContext){
                List<ColumnExpr> partitionByExprs = (List<ColumnExpr>) visit(partitionByClauseContext.columnExprList());
                partitionByClause = new PartitionByClause(partitionByExprs);
            }
            ClickHouseParser.WinOrderByClauseContext orderByClauseContext = ctx.windowExpr().winOrderByClause();
            if(orderByClauseContext != null){
                List<OrderExpr> orderExprs = (List<OrderExpr>) visit(orderByClauseContext.orderExprList());
                orderByClause = new OrderByClause(orderExprs);
            }
        };
        WindowExpr windowExpr = new WindowExpr(partitionByClause, orderByClause);
        return ColumnExpr.createWinFunction(identifier, windowExpr);
    }
    ...
    public Object visitColumnExprCast(ClickHouseParser.ColumnExprCastContext ctx) {
        ...
        if(columnTypeExpr instanceof ParamColumnTypeExpr){
            ParamColumnTypeExpr paramColumnTypeExpr = (ParamColumnTypeExpr) columnTypeExpr;
            args.add(ColumnExpr.createFunction(paramColumnTypeExpr.getName(), null, paramColumnTypeExpr.getParams()));
        }else{
    ...
    public Object visitColumnExprFunction(ClickHouseParser.ColumnExprFunctionContext ctx) {
        ...
        if (null != ctx.DISTINCT() && null != name && name.getName().toUpperCase().equals("COUNT")) {
    ...
    public Object visitTableArgExpr(ClickHouseParser.TableArgExprContext ctx) {
        ...
        if (null != ctx.nestedIdentifier()) {
            TableIdentifier tableIdentifier = (TableIdentifier) visit(ctx.nestedIdentifier());
