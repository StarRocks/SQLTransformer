...
import com.tme.di.parser.ast.DistributedTableInfoDetector;
...
public class TestAstParser {
    ...
    public void testReferredTablesDetector() {
        String sql = "/* mixquery trace_id: query_unique_key:simple_CLICK_HOUSE_3204448659726996147 biz_id:a */select preference, sum(usernum) from table group by preference order by case when preference = '轻度' then 1 when preference = '中度' then 2 else 3 end as rank asc LIMIT 5000";
        AstParser astParser = new AstParser();
    ...
    @Test
    public void testSearchTablePartitions() {
        String sql = "/* mixquery trace_id: query_unique_key:simple_CLICK_HOUSE_3204448659726996147 biz_id:a */select preference, sum(usernum) from table group by preference order by case when preference = '轻度' then 1 when preference = '中度' then 2 else 3 end as rank asc LIMIT 5000";
        AstParser astParser = new AstParser();
        Object ast = astParser.parse(sql);
        ReferredTablesDetector referredTablesDetector = new ReferredTablesDetector();
